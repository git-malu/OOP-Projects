...
public abstract class ABall implements Observer {
	 ...
	 * @param c color
	 */
	ABall(Point l, int r, Point v, Color c) {
		this.location = l;
		this.radius = r;
		this.velocity = v;
		this.color = c;
	 ...
	 */
	public void bounce() {
		if (location.x < 0 || location.x + 2 * radius > BallModel.returnBounds().getWidth()) {
			velocity.x = -velocity.x;
			location.x = location.x + 2 * velocity.x;
		}
		if (location.y < 0 || location.y + 2 * radius > BallModel.returnBounds().getHeight()) {
